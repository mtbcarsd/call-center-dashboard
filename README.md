...
Страницы: 📊 Аналитика · 📁 Звонки (галерея, плеер с перемоткой по репликам,
инлайн-редакторы, теги/коллекции, комментарии) · 🧑‍💼 Операторы · 🏆 Рейтинг ·
🛡️ Compliance · 📈 Тренды · 👥 Команда · 🙋 Мой кабинет (только employee, свои
звонки + LLM-рекомендации по обучению через `GET /coaching/{operator_name}`).
...
поэтому большинство их метрик впрямую не переносится. Но три UX-идеи стоят своего места в очереди,
как только дойдут руки (низкий приоритет, обсуждались, но не начаты на 2026-07-18):
- 📖 Страница-глоссарий «Показатели — что означают» — формулы наших метрик простым языком
  (`agent_performance_score`, `qa_score`, % прохождения чек-листа и т.д.)
...
- 🧭 Footer-бар на вкладках: активные фильтры + актуальность данных (особенно полезно, пока
  все звонки — из одного batch-прогона, это легко забыть)
