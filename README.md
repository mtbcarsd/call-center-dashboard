...
`audio_original_data/` остаётся **только локально** (`.gitignore`) и не публикуется —
изначально 10 настоящих звонков клиентов банка (5 ОО + 5 ОРККиП), с сессии 9
(2026-07-18) туда же добавлены ещё 10 **синтетических** (по 5 на отдел, именованные
вымышленные операторы, реальный `call_datetime`), сгенерированных тем же способом,
что и ниже — `edge-tts` через `scripts/add_new_calls.py`. В репозиторий для
WER-тестирования вместо реальных записей включены отдельные **синтетические** звонки
(`test_data/`, сгенерированы `edge-tts`: оператор — `ru-RU-SvetlanaNeural`, клиент —
`ru-RU-DmitryNeural`) — 6 файлов, суммарно 5.4 минуты, покрывают темы
кредиты/карты/переводы/жалобы/другое, один файл — в телефонном качестве 8kHz
(`kredit_nalichnymi_8khz.wav`).
...
## Dash-дашборд (основной публичный дашборд)

Миграция со Streamlit на Plotly Dash (см. [DASH_MIGRATION_PLAN.md](DASH_MIGRATION_PLAN.md))
завершена 2026-07-24 (фазы D1-D5) — это теперь основной публичный дашборд,
с ролевым доступом (executive/manager/employee) и полным паритетом со
Streamlit-версией, включая вкладку «Звонки» (плеер, инлайн-редакторы, теги/
коллекции, комментарии). После cutover (2026-07-24 — 2026-07-27) добавлена
светлая/тёмная тема, страница «Справка», и три раунда доработок по мотивам
референсных дашбордов (geckoboard.com, `dash-manufacture-spc-dashboard`,
`dash-clinical-analytics`) — тренды/алерты на Аналитике (E), control chart и
спарклайны на Операторах (F), annotated heatmap на Compliance (G). 122
теста, 1 skip.

**URL:** https://dash-director-production.up.railway.app
**Учётки:** `julia` (executive, все отделы) · `boss_oo`/`boss_orkki` (manager,
свой отдел) · `sokolova` (employee, личный кабинет) — пароли в
`scripts/seed_users.py`.

Страницы:
- 📊 **Аналитика** — hero-KPI (крупный план на 2 главных метриках), лидерборд
  топ-3/анти-топ-3 операторов, проактивные алерты по пороговым эвристикам
  (без LLM), тренды по дням (7-дневное скользящее среднее) с фильтром периода
  (7/30/90 дней/всё время) и сравнением к предыдущему периоду, автообновление
  раз в 90 сек
- 📁 **Звонки** — галерея (с фильтром «только с аудио»), плеер с перемоткой по
  репликам, инлайн-редакторы, теги/коллекции, комментарии
- 🧑‍💼 **Операторы** — спарклайны тренда оценки + control chart (норма команды
  ±1.5σ) в стиле Shewhart-карт, донат по проблемным звонкам, quick-stats
  шапка, drill-in по клику на оператора (в т.ч. по ссылке из «Рейтинга»/
  Compliance)
- 🏆 **Рейтинг** · 🛡️ **Compliance** (annotated heatmap нарушений по
  дням/неделям с фильтром по типу нарушения) · 📈 **Тренды** (LLM-агент) ·
  👥 **Команда** · 🙋 **Мой кабинет** (только employee, свои звонки + LLM-
  рекомендации по обучению через `GET /coaching/{operator_name}`) · ❓
  **Справка** (тур по страницам + честное сравнение с конкурентом, видна всем
  ролям)

## Streamlit-дашборд (легаси, доступен для отката)

Исходная Streamlit-версия оставлена работать без изменений как страховка на
случай отката — мягкий cutover вместо полного отключения (см. Phase D5 в
плане миграции), срок отключения не назначен.
...
|---|---|---|
| `dash-director` (Dash) | ✅ работает | **Основной публичный дашборд** — D1-D5 завершены (2026-07-24), роли executive/manager/employee, полный паритет со Streamlit включая вкладку «Звонки» |
| `call-center-dashboard` (Streamlit) | ✅ работает | Легаси, доступен для отката — дальше не развивается |
| `openwebui` | ⚠️ работает, но без LLM | Сам сервис жив, но настроен на `pipelines` (см. ниже), который не отвечает — чат нефункционален |
| `Postgres` | ✅ работает | Общая база для Dash, Streamlit и сервисной архитектуры |
...
**завершена 2026-07-24** (фазы D1-D5). Детальный фазовый план и статус —
[DASH_MIGRATION_PLAN.md](DASH_MIGRATION_PLAN.md). Дальнейшее развитие функционала
(Phase 2-5 ниже) теперь идёт в Dash-версии, а не в Streamlit.

✅ **Пост-миграционная доработка Dash-дашборда по мотивам референсных дашбордов**
(2026-07-24 — 2026-07-27, вне исходного плана D1-D5): светлая/тёмная тема,
страница «Справка», фильтр «только с аудио» на «Звонках»; Фаза E (Аналитика —
тренды по дням, фильтр периода со сравнением, hero-KPI, лидерборд, пороговые
алерты без LLM); Фаза F (Операторы — спарклайны + control chart в стиле
`dash-manufacture-spc-dashboard`, донат по проблемным звонкам, quick-stats
шапка); Фаза G (Compliance — annotated heatmap нарушений в стиле
`dash-clinical-analytics`, фильтры); плюс drill-in по имени оператора из
таблиц «Рейтинг»/Compliance на control chart. Никакого отдельного планового
документа под эти фазы не заводили — не завязано на явный план, как D1-D5.

Streamlit-дашборд (легаси) дальше не развивается, но описание уже реализованных
там Phase 1-3 ниже остаётся как справка — весь этот функционал уже есть и в
Dash-версии (D1-D4). Оставшиеся нереализованные пункты (Phase 4-5) — кандидаты
на реализацию в Dash, если понадобятся.

Изначальный план эволюции в сторону возможностей уровня
...
- 🛡️ Compliance-вкладка — общий % прохождения, список звонков с нарушениями, разбивка по оператору
  (изначально без графика «по времени» — все звонки были из одного batch-прогона; с сессии 9
  у 10 из 20 звонков есть реальный `call_datetime`, график по времени пока не добавлен, см. NEXT_SESSION.md)
...
- 📈 Вкладка Тренды — дашборд дёргает `GET /trends` на сервисе `api` (LLM-агент, Groq), слайдер
  «сколько последних звонков анализировать», кеш на 5 минут, кнопка принудительного обновления

**Phase 4 — интеграции:**
- 💬 Встроенный AI-чат (iframe на OpenWebUI или прямая ссылка) вместо двух разрозненных интерфейсов —
  пока не работает: не из-за RAM (Hobby-план это снял), а из-за несовместимости
  torch/torchvision/torchaudio в базовом образе `pipelines`, см. «Статус деплоя»
...
**Идеи из референса «Dashboard Call Center» (Power BI, операционные метрики трафика):**

Пользователь показал инструкцию/техдокументацию похожего дашборда. Это **другой класс данных** —
телефонные CDR (сколько звонков поступило/принято/брошено, % дозвона за 60с, среднее время ответа) —
у нас такого источника нет (наш пайплайн анализирует уже записанные звонки, не живой телефонный трафик),
поэтому большинство их метрик впрямую не переносится. Из трёх обсуждавшихся UX-идей одна теперь
частично закрыта, две остаются в очереди (низкий приоритет):
- ✅ ~~Дневной разрез~~ — частично сделано в рамках Фазы E (2026-07-26): тренды по дням
  (оценка/% решено/объём) с фильтром периода на «Аналитике», используя ранее неиспользованные
  320 синтетических звонков с реальным `call_datetime`. Часовой разрез (`WYNIKI GODZINOWE`) и
  явный график объёма звонков-по-часам всё ещё не сделаны.
- 🚦 Явные целевые пороги (напр. чек-лист ≥ 70%) с красным/зелёным индикатором построчно
  в «Compliance»/«Рейтинге», а не только подсветка худшего пункта
- 🧭 Footer-бар на вкладках: активные фильтры + актуальность данных

Страница-глоссарий «Показатели — что означают» больше не отдельный пункт — покрыта страницей
«Справка» (добавлена 2026-07-24).
