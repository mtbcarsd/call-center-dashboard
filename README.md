...
|---|---|---|
| `call-center-dashboard` (Streamlit) | ✅ работает | Основной публичный дашборд, Phase 1 галереи задеплоена |
...
**Phase 2 — организация данных:**
- 🎙 Дикторы — переименование "Оператор/Клиент" в реальные имена, статистика по оператору
- 🏷️ Теги и коллекции — ручная разметка звонков (напр. "спорные кейсы")
- 📝 Комментарии/заметки под транскриптом
- ✅ Ручная валидация AI-категорий — принять/отклонить категорию, предложенную классификатором,
  прямо на карточке звонка (не просто доверять LLM молча)

**Phase 3 — то, что уже посчитано, но не показано:**
- 🛡️ Compliance-вкладка с трендами нарушений по времени/оператору (сейчас — только иконка на карточке)
- 📈 Вкладка Тренды — `agents/trends.py` уже существует и работает (`GET /trends`), но не подключён к Streamlit
- 🏆 Рейтинг операторов по чек-листу с разбивкой по пунктам (не только итоговым баллом)
- 🧑‍⚖️ Ручная QA-оценка отдельно от AI-оценки — колонка "оценка проверяющего" рядом с
  `agent_performance_score`, с расхождением между ними (доверяй, но проверяй)
