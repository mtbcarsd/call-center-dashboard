...
### J8 — Тестирование, история и финал ✅ (2026-07-28)
- `docs/testing.md`: структура `tests/` (123 теста, 1 skip — реальный
  Ollama-smoke), почему без Selenium/`dash.testing` (callback'и — чистые
  функции), явная оговорка о непокрытых `clientside_callback`, историческая
  сноска про `AppTest` для легаси Streamlit, WER-тестирование
- `docs/history.md`: сжатая хронология трёх ключевых переходов (хранилище
  Snowflake→SQLite→Postgres, LLM Ollama→Groq, дашборд Streamlit→Dash) — не
  построчный пересказ журнала памяти, ссылка на README/NEXT_SESSION.md за
  актуальным бэклогом вместо дублирования
- Финальная сборка всей книги (`jupyter-book build --html --strict`) —
  чисто, без предупреждений на всех 10 страницах
- `index.md` обновлён: убрана пометка «черновики», все разделы отмечены
  завершёнными
- README.md и NEXT_SESSION.md обновлены — книга отмечена готовой, добавлена
  команда локальной сборки, добавлено напоминание обновлять `docs/` по месту
  при будущих изменениях в проекте
...
- ~~Не настраиваем публикацию (GitHub Pages/Railway) в рамках этого плана~~ —
  решение пересмотрено сразу после J8 (2026-07-28) по явной просьбе
  пользователя «запушь и задеплой на railway». Заведён отдельный сервис
  `docs` (`docs/Dockerfile`, статический экспорт + `python -m http.server`),
  автодеплой при пуше в `main`, как у остальных сервисов:
  https://docs-production-f5b7.up.railway.app. Два реальных фикса по пути —
  jupyter-book (mystmd) требует Node ≥18 (nodeenv виснет на интерактивном
  вопросе без стандартного ввода) и `procps`/`ps` для штатной остановки
  временного dev-сервера после статического экспорта — оба не воспроизводились
  локально, где Node уже стоял в системе
